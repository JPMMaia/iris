...
In the Iris programming language we have `function_constructor` and `type_constructor`. These are functions that are called at compile time to create new functions / types.
...
2. Deduce function constructor arguments of needed (e.g. replace `iris::Call_expression` by a `iris::Instance_call_expression` with all deduced constructor arguments)
...
6. Replace `iris::Instance_call_expression` with a corresponding `iris::Call_expression`
