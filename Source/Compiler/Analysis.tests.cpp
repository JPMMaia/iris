...
namespace iris::compiler
{
    ...
    static void collect_lambda_expressions(
        iris::Statement const& statement,
        std::pmr::vector<iris::Lambda_expression const*>& lambdas
    );

    static void collect_lambda_expressions_from_expression(
    ...
    )
    {
        ...
        {
            iris::Lambda_expression const& lambda = std::get<iris::Lambda_expression>(expression.data);
            lambdas.push_back(&lambda);
            collect_lambda_expressions(lambda.body, lambdas);
        }
        else if (std::holds_alternative<iris::Block_expression>(expression.data))
        {
            iris::Block_expression const& block = std::get<iris::Block_expression>(expression.data);
            for (iris::Statement const& nested_statement : block.statements)
                collect_lambda_expressions(nested_statement, lambdas);
        }
        else if (std::holds_alternative<iris::If_expression>(expression.data))
        {
            iris::If_expression const& data = std::get<iris::If_expression>(expression.data);
            for (iris::Condition_statement_pair const& serie : data.series)
            {
                if (serie.condition.has_value())
                    collect_lambda_expressions(serie.condition.value(), lambdas);

                for (iris::Statement const& nested_statement : serie.then_statements)
                    collect_lambda_expressions(nested_statement, lambdas);
            }
        }
    }
    ...
    )
    {
        for (iris::Expression const& expression : statement.expressions)
            collect_lambda_expressions_from_expression(expression, lambdas);
    }
    ...
    )
    {
        ...
        for (auto const& definition : module.definitions.function_definitions)
        {
            ...
            for (auto const& statement : definition.statements)
                collect_lambda_expressions(statement, lambdas);
        }
            ...
            for (auto const& statement : definition.statements)
            {
                for (auto const& expr : statement.expressions)
                {
                    if (std::holds_alternative<iris::Variable_declaration_with_type_expression>(expr.data))
                    {
                        ...
                        if (var_expr.name == variable_name &&
...
export function main() -> ()
{
    var mapper = lambda(x: Int32) -> Bool => x * 2;
        ...
        test_analysis_has_diagnostic(input, "Type mismatch");
    }
...
export function main() -> ()
{
    var offset: Int32 = 10;
    var scale: Int32 = 2;
    var cmp: Comparator = lambda(a, b) => (a + offset) * scale - b;
}
)";
...
export function main() -> ()
{
    ...
    var outer: Outer = lambda(a, b) => {
        ...
        // `a` is the outer lambda's own parameter, so the outer lambda captures only `base`.
        ...
        REQUIRE(outer_captured.size() == 1);
        CHECK(outer_captured[0] == "base");

        // The inner lambda captures `a` from the outer lambda and `base` from main.
        ...
        REQUIRE(inner_captured.size() == 2);
        CHECK(std::find(inner_captured.begin(), inner_captured.end(), "a") != inner_captured.end());
        CHECK(std::find(inner_captured.begin(), inner_captured.end(), "base") != inner_captured.end());
    }
...
export function main() -> ()
{
    var add: Adder = lambda(a: Float32, b: Int32) => a + b;
        ...
        test_analysis_has_diagnostic(input, "Lambda parameter 'a' type mismatch");
    }
...
export function main() -> ()
{
    var mapper = lambda(x: Int32) -> Int32 => 1.0f32;
}
)";
