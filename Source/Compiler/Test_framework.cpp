...
namespace h::compiler
{
    ...
    )
    {
        ...
        for (h::Module const* const core_module : core_modules)
        {
            ...
            add_test_infos(core_module->name, module_alias_name, core_module->export_declarations.function_declarations);
            add_test_infos(core_module->name, module_alias_name, core_module->internal_declarations.function_declarations);
        }
...
{{
    return &g_tests[0];
        ...
        std::string const test_module_code = std::format(test_template, test_module_name, imports, test_names, test_pointers, test_infos.size());
