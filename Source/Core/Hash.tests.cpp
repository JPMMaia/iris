...
namespace
{
    // Minimal scope guard: runs the given action when the enclosing scope exits.
    ...
    )
    {
        if (input_names.empty())
        {
            for (std::size_t i = 0; i < input_types.size(); ++i)
                input_names.push_back(std::pmr::string{ "param" + std::to_string(i) });
        }
        if (output_names.empty())
        {
            for (std::size_t i = 0; i < output_types.size(); ++i)
                output_names.push_back(std::pmr::string{ "result" + std::to_string(i) });
        }
    ...
    static XXH64_state_t* create_hash_state()
    {
        ...
        CHECK(XXH64_reset(state, 0) == XXH_OK);
        return state;
    }
}
...
TEST_CASE("Hashes Lambda_type with single input and output type", "[Core][Hash]")
{
    ...
    auto const input_type = iris::create_integer_type_type_reference(32, true);
    auto const output_type = create_fundamental_type(iris::Fundamental_type::Bool);

    auto const lambda_type = create_lambda_type(
        std::pmr::vector<iris::Type_reference>{ std::move(input_type) },
        std::pmr::vector<iris::Type_reference>{ std::move(output_type) }
    );
...
TEST_CASE("Hashes Lambda_type with multiple input and output types", "[Core][Hash]")
{
    ...
    std::pmr::vector<iris::Type_reference> input_types{
        iris::create_integer_type_type_reference(32, true),
...
TEST_CASE("Different Lambda_type produce different hashes", "[Core][Hash]")
{
    ...
    auto const lambda_type_a = create_lambda_type(
        std::pmr::vector<iris::Type_reference>{ iris::create_integer_type_type_reference(32, true) },
        std::pmr::vector<iris::Type_reference>{ create_fundamental_type(iris::Fundamental_type::Bool) }
    );

    auto const lambda_type_b = create_lambda_type(
        std::pmr::vector<iris::Type_reference>{ create_fundamental_type(iris::Fundamental_type::Float64) },
        std::pmr::vector<iris::Type_reference>{ create_fundamental_type(iris::Fundamental_type::Bool) }
    );
...
TEST_CASE("Same Lambda_type produces same hash (deterministic)", "[Core][Hash]")
{
    ...
    auto const input_type2 = iris::create_integer_type_type_reference(32, true);
    auto const output_type2 = create_fundamental_type(iris::Fundamental_type::Bool);

    auto const lambda_type2 = create_lambda_type(
        std::pmr::vector<iris::Type_reference>{ std::move(input_type2) },
        std::pmr::vector<iris::Type_reference>{ std::move(output_type2) }
    );
...
TEST_CASE("Lambda_declaration with unique_name produces different hash", "[Core][Hash]")
{
    ...
    auto const input_type = iris::create_integer_type_type_reference(32, true);
    auto const output_type = iris::create_integer_type_type_reference(32, true);

    auto const lambda_decl_a = iris::Lambda_declaration{
        .name = std::pmr::string{ "Comparator" },
        .unique_name = std::nullopt,
        .input_parameter_types = std::pmr::vector<iris::Type_reference>{ std::move(input_type) },
        .output_parameter_types = std::pmr::vector<iris::Type_reference>{ std::move(output_type) },
    };

    auto const input_type2 = iris::create_integer_type_type_reference(32, true);
    auto const output_type2 = iris::create_integer_type_type_reference(32, true);

    auto const lambda_decl_b = iris::Lambda_declaration{
        .name = std::pmr::string{ "Comparator" },
        .unique_name = std::pmr::string{ "Comparator_001" },
        .input_parameter_types = std::pmr::vector<iris::Type_reference>{ std::move(input_type2) },
        .output_parameter_types = std::pmr::vector<iris::Type_reference>{ std::move(output_type2) },
    };
...
TEST_CASE("Hashes Lambda_declaration with multiple output parameters", "[Core][Hash]")
{
    ...
    std::pmr::vector<std::pmr::string> output_names{
        std::pmr::string{ "new_x" },
        std::pmr::string{ "new_y" },
        std::pmr::string{ "new_z" },
    };
